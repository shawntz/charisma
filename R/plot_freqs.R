plot_freqs <- function(charisma_obj) {
  hex <- get_mapped_hex()
  color_summary <- hex %>% rename(classification = color.name) %>% right_join(charisma_obj$charisma_calls_table, by = "classification")
  freq_bar <- barplot(height = color_summary$prop, names = color_summary$classification, col = color_summary$default.hex,
                      main = paste0("Color Frequency (k=", charisma_obj$k, ", ", (charisma_obj$prop_threshold*100), "%)"),
