plot_original <- function(charisma_obj, mar = c(0, 0, 5, 0)) {
  ...
  plot(0:1, 0:1, type = "n", axes = FALSE, asp = asp, main = "Original", xlab = "", ylab = "")
...
plot_recolored <- function(img, mar = c(0, 0, 5, 0)) {
  ...
  plot(0:1, 0:1, type = "n", axes = FALSE, asp = asp, main = "Recolored", xlab = "", ylab = "")
...
plot_masked <- function(img, mar = c(0, 0, 5, 0)) {
  par(mar = mar)
  asp <- dim(img$original_img)[1] / dim(img$original_img)[2]
  plot(0:1, 0:1, type = "n", axes = FALSE, asp = asp, main = "Masked", xlab = "", ylab = "")
  graphics::rasterImage(img$color_mask, 0, 0, 1, 1)
}


plot_props <- function(charisma_obj, use.default.bar.colors = TRUE, mar = c(5.5, 8, 5, 0)) {
  par(mar = mar)
  ...
  freq_bar <- barplot(height = cluster_specific_hex_vals$prop,
                      ...
                      col = bar_colors,
                      main = paste0("(k = ", charisma_obj$k, ", ", (charisma_obj$prop_threshold*100), "%)                   "),
                      ylim = c(0,1), ylab = "Proportion of Image\n", las = 2)

  if (charisma_obj$prop_threshold > 0) {
    abline(h = charisma_obj$prop_threshold, col = "red", lty = "dashed", lwd = 2)
  }
}


plot_pavo_input <- function(charisma_obj, mar = c(0, 0, 5, 0)) {
  par(mar = mar)
  asp <- dim(charisma_obj$original_img)[1] / dim(charisma_obj$original_img)[2]
  plot(0:1, 0:1, type = "n", axes = FALSE, asp = asp, main = "Pavo", xlab = "", ylab = "")
  graphics::rasterImage(charisma_obj$input2pavo, 0, 0, 1, 1)
}


plot_pavo_pal <- function(charisma_obj, mar = c(0, 5, 5, 5)) {
  par(mar = mar)
  asp <- dim(charisma_obj$original_img)[1] / dim(charisma_obj$original_img)[2]
  palette <- charisma_obj$pavo_adj_class_plot_cols
  image(seq_along(palette), 1, as.matrix(seq_along(palette)),
        col = palette,
        main = paste0("Pavo Classes (k = ", charisma_obj$k, ")"),
