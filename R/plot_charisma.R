...
#' @export
plot.charisma <- function(x, plot.all = T, plot.original = F, plot.recolored = F, plot.masked = F, plot.props = F, plot.pavo.img = F, plot.pavo.classes = F, font.size = 1.75, real.bar.colors = T) {
  # reset graphical env
  current_par <- graphics::par(no.readonly = TRUE)
  on.exit(graphics::par(current_par))

  # set new graphical env defaults
  par(cex.main = font.size, cex.lab = font.size, cex.axis = font.size, mar = c(10,1,8,1))
  ...
  # construct layouts
  if (plot.all) {
    plot_layout <- matrix(c(0, 1, 4, 0, 0, 2, 5, 0, 0, 3, 6, 0), ncol = 3, nrow = 4)
    plot_heights <- rep(c(0.15, 0.35, 0.35, 0.15), times = 3)
  } else {
    num_plots <- sum(c(plot.original, plot.recolored, plot.masked, plot.props, plot.pavo.img, plot.pavo.classes))
    if (num_plots == 0) {
